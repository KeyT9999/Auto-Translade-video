...
def _generate_metadata_groq(prompt: str) -> str | None:
    import os
    import requests
    groq_key = os.getenv("GROQ_API_KEY", "")
    if not groq_key:
        logger.warning("GROQ_API_KEY not set. Skipping Groq metadata generation.")
        return None
    try:
        logger.info("Calling Groq Llama 3.3 70B for metadata generation...")
        headers = {
        ...
        payload = {
            ...
            "messages": [
                {
                    "role": "system",
                    "content": "You are a precise JSON YouTube metadata assistant."
                },
                {
                    "role": "user",
                    "content": prompt
...
{{"title": "...", "description": "...", "hashtags": ["#tag1", "#tag2", ...]}}"""
    ...
    if not use_gemini:
        ...
        try:
            ...
            logger.error(f"Gemini metadata generation failed: {e}. Trying Groq fallback...")
            text = _generate_metadata_groq(prompt)

    if not text:
        logger.error("Failed to generate YouTube metadata from both Gemini and Groq.")
